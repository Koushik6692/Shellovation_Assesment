...
const Checkout = () => {
    ...
    try {
      ...
      const productPromises = Object.values(groupedItems).map(async (item) => {
        const productResponse = await fetch(
          `https://ecommercebackend-8gx8.onrender.com/product/${item.productId}`
        );
  ...
  const handlePlaceOrder = async () => {
    ...
    if (saveAddress) {
      try {
        await fetch(
          "https://ecommercebackend-8gx8.onrender.com/update-address",
          {
            method: "POST",
    ...
    try {
      const response = await fetch(
        "https://ecommercebackend-8gx8.onrender.com/place-order",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="saveAddress">
                Save this for future preference
              </label>
            </div>
          </div>
        </div>
